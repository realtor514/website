# -*- coding: utf-8 -*-
"""Carrousel Instagram 1080x1350 pour l inscription 35 terrasse Jacques-Leonard.

Meme gabarit que le carrousel du 28 rue St-Hilaire: 8 slides JPG numerotes
dans l ordre de publication, couleurs echantillonnees dans le logo
RE/MAX DU CARTIER. Inscription en collaboration avec l Equipe Pistoli:
les deux courtiers apparaissent sur la slide de contact.

Les photos viennent de carrousel-instagram/sources/.
...
CARROUSEL = os.path.dirname(HERE)
ROOT = os.path.dirname(CARROUSEL)
SOURCES = os.path.join(CARROUSEL, "sources")
FONTS = os.path.join(CARROUSEL, "fonts")
...
# ---------------------------------------------------------------- inscription
ADRESSE_1 = "35, terrasse"
ADRESSE_2 = "Jacques-Léonard"
SECTEUR = "Rivière-des-Prairies, Montréal"
PRIX = "380 000 $"
CENTRIS = "15815581"
...
SOUS_SOL = "11-Salon sous sol.jpg"
DECK = "14-deck extérieur.png"
COUR = "15-backyard.png"
DOMAINE = "16-parc et piscine privés du domaine.png"

# la photo du domaine est un montage de trois cliches: on decoupe les zones
PARC = (0.0, 0.0, 1.0, 0.552)           # bande du haut, le parc au couchant
AERIEN = (0.0, 0.567, 0.489, 1.0)       # vue aerienne, piscine et riviere

# la porte de garage blanche tombe pile sous le titre de la couverture: on
# recadre a gauche pour poser le titre sur la haie et la brique, plus sombres
FACADE_CADRAGE = (0.0, 0.0, 0.66, 1.0)
...
# ---------------------------------------------------------------- images
def cover(path, w, h, fy=0.5, crop=None):
    """Recadre en remplissant la boite. fy = centre d interet vertical.

    crop = zone relative (g, h, d, b) prelevee avant le recadrage, utile
    pour extraire un cliche d un montage. Accepte un chemin ou une image.
    """
...
def photo(canvas, name, box, r=22, shad=True, fy=0.5, crop=None):
    x0, y0, x1, y1 = box
    w, h = x1 - x0, y1 - y0
    if shad and r:
        shadow(canvas, box, r)
    im = cover(os.path.join(SOURCES, name), w, h, fy, crop).convert("RGBA")
    if r:
...
ICONS = {
    ...
    "stairs": [("p", [(2.5, 21), (8, 21), (8, 15), (14, 15), (14, 9),
                      (20, 9), (20, 3), (21.5, 3)])],
...
# ================================================================ SLIDE 1
def slide1():
    """Couverture: la facade, le titre blanc pose au bas de la photo,
    pastille navy NOUVEAUTE, puis bandeau blanc avec le prix."""
    c = base()
    photo(c, FACADE, (0, 0, W, 900), r=0, shad=False, fy=0.08,
          crop=FACADE_CADRAGE)
    gradient(c, 330, 900, 246)
    gradient(c, 430, 900, 210)
    d = ImageDraw.Draw(c)

    d.text((M, 470), ADRESSE_1, font=playfair(78, 700), fill=WHITE)
    d.text((M, 566), ADRESSE_2, font=playfair(78, 700), fill=WHITE)
    d.text((M, 678), SECTEUR, font=inter(32, 400), fill=(255, 255, 255, 225))
...
def slide2():
    ...
    d.text((M, 134), "Une maison de ville", font=playfair(72, 700), fill=NAVY)
    d.text((M, 218), "au Domaine Bonneville", font=playfair(72, 700), fill=NAVY)

    photo(c, SALLE_A_MANGER, (M, 336, W - M, 856))

    items = [("bed", "4", "Chambres, 1 au sous-sol"),
             ("bath", "2 + 1", "Salle de bain / eau"),
             ("car", "Garage", "Allée à usage exclusif"),
             ("stairs", "3 niveaux", "9 pièces, sous-sol complet")]
    ...
    tracked(d, (M + 34, 1204), "PRIX DEMANDÉ", inter(23, 800), NAVY_SOFT, 5)
    d.text((W - M - 34, 1198), PRIX, font=playfair(58, 800), fill=WHITE,
           anchor="ra")
    return c
...
def slide3():
    c = base()
    photo(c, CUISINE, (0, 0, W, 700), r=0, shad=False, fy=0.55)
    d = ImageDraw.Draw(c)

    y = 756
    eyebrow(c, (M, y), "LA CUISINE")
    y += 46
    d.text((M, y), "Une cuisine claire", font=playfair(66, 700), fill=NAVY)
    d.text((M, y + 78), "et fonctionnelle", font=playfair(66, 700), fill=NAVY)
    y += 196

    for t in ["Armoires blanches et dosseret en mosaïque",
              "Grande fenêtre au-dessus de l'évier",
              "Espace pour un îlot ou un coin repas"]:
...
def slide4():
    c = base()
    photo(c, DOMAINE, (0, 0, W, 700), r=0, shad=False, fy=0.5, crop=PARC)
    d = ImageDraw.Draw(c)

    y = 756
    eyebrow(c, (M, y), "LE DOMAINE BONNEVILLE")
    y += 46
    d.text((M, y), "Tout un milieu de vie", font=playfair(68, 700), fill=NAVY)
    y += 122

    for t in ["Piscine et pataugeoire privées",
        ...
        icon(c, "check", (M, y + 2), 34, NAVY + (255,))
        d.text((M + 52, y), t, font=inter(28, 500), fill=INK)
        y += 62
...
def slide5():
    ...
    d.text((M, 134), "Chaque étage compte", font=playfair(70, 700), fill=NAVY)

    photo(c, CHAMBRE, (M, 268, W - M, 700), fy=0.55)
    cw = (W - 2 * M - 24) // 2
    photo(c, SOUS_SOL, (M, 724, M + cw, 1028))
    photo(c, DECK, (M + cw + 24, 724, W - M, 1028), fy=0.55)

    d.text((M, 1058), "Trois niveaux habitables", font=inter(25, 700), fill=NAVY)
    para(d, (M, 1102),
         "Chambre principale à l'étage, salon au sous-sol "
         "et terrasse couverte à l'arrière.",
         inter(28, 400), INK, W - 2 * M, 42)
...
# ================================================================ SLIDE 6
def slide6():
    """Vue aerienne du secteur, prelevee dans le montage du domaine."""
    c = base()
    photo(c, DOMAINE, (0, 0, W, 700), r=0, shad=False, fy=0.5, crop=AERIEN)
    d = ImageDraw.Draw(c)

    y = 756
    eyebrow(c, (M, y), "RIVIÈRE-DES-PRAIRIES")
    y += 46
    d.text((M, y), "Un quartier", font=playfair(66, 700), fill=NAVY)
    d.text((M, y + 78), "bien desservi", font=playfair(66, 700), fill=NAVY)
    y += 196

    for ic, t in [("pin", "Écoles primaire, secondaire et cégep"),
                  ("tree", "Parcs, pistes cyclables et bord de l'eau"),
                  ("train", "Transport en commun et autoroute 40")]:
        icon(c, ic, (M, y - 2), 40, NAVY + (255,))
        d.text((M + 60, y), t, font=inter(28, 500), fill=INK)
        y += 62
...
def slide7():
    ...
    d.text((W // 2, 972), "vivre ici.", font=playfair(80, 700), fill=WHITE,
           anchor="ma")
    tracked(d, (W // 2, 1094),
            "35, TERRASSE JACQUES-LÉONARD   ·   " + PRIX.upper(),
            inter(23, 700), (255, 255, 255, 215), 3, "c")
...
def slide8():
    ...
    s, gap = 250, 60
    x0 = (W - (2 * s + gap)) // 2
    circle_photo(c, os.path.join(SOURCES, PORTRAIT_ROVENA), (x0, 86), s, fy=0.0)
    circle_photo(c, studio_portrait(os.path.join(SOURCES, PORTRAIT_GEORGES)),
    ...
    tracked(d, (W // 2, 1296), "CENTRIS " + CENTRIS, inter(21, 600), GREY, 4,
            "c")
    return c
...
if __name__ == "__main__":
    ...
    for fn, name in [
            ...
            (slide3, "03-cuisine.jpg"), (slide4, "04-domaine.jpg"),
