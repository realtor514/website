# -*- coding: utf-8 -*-
"""Carrousel Instagram 1080x1350 pour l inscription 4071 rang Saint-Hyacinthe.

Meme gabarit que les carrousels du 28, rue St-Hilaire et du 35, terrasse
Jacques-Leonard: 8 slides JPG numerotes dans l ordre de publication,
couleurs echantillonnees dans le logo RE/MAX DU CARTIER. Inscription en
collaboration avec l Equipe Pistoli: les deux courtiers apparaissent sur la
slide de contact.
...
# ---------------------------------------------------------------- inscription
ADRESSE_1 = "4071, rang"
ADRESSE_2 = "Saint-Hyacinthe"
ADRESSE_1L = "4071, RANG SAINT-HYACINTHE"
SECTEUR = "Saint-Hermas, Mirabel"
PRIX = "449 000 $"
CENTRIS = "26269222"

# photos deposees dans sources/
FACADE = "01-Facade principale.png"
SALON = "04-Salon - RDC.png"
CUISINE = "06-Cuisine - RDC.png"
CHAMBRE = "08-Chambre principale - RDC.png"
POELE = "11-Escalier vers le 2e etage - RDC.png"
CHAMBRE_PIN = "17-Chambre 4 - 2e etage.png"
GRANGE = "19-Grange - vue exterieure.png"
...
def cover(path, w, h, fy=0.5, crop=None):
    ...
    crop = zone relative (g, h, d, b) prelevee avant le recadrage. Accepte
    un chemin ou une image deja ouverte.
    """
...
ICONS = {
    ...
    "land": [("p", [(3, 8), (3, 3), (8, 3)]), ("p", [(16, 3), (21, 3), (21, 8)]),
             ...
             ("l", 8, 12, 16, 12)],
    "barn": [("p", [(2, 10.5), (12, 3), (22, 10.5)]), ("l", 3.6, 10.5, 3.6, 21),
             ("l", 20.4, 10.5, 20.4, 21), ("l", 3.6, 21, 20.4, 21),
             ("r", 9.2, 14.5, 14.8, 21, 1)],
...
def slide1():
    ...
    c = base()
    photo(c, FACADE, (0, 0, W, COUV_BAS), r=0, shad=False, fy=0.5)
    gradient(c, 330, COUV_BAS, 246)
...
def slide2():
    ...
    d.text((M, 134), "Une maison de 1935", font=playfair(72, 700), fill=NAVY)
    d.text((M, 218), "sur 22 152 pi²", font=playfair(72, 700), fill=NAVY)

    photo(c, SALON, (M, 336, W - M, 856), fy=0.55)

    items = [("bed", "4", "Chambres"),
             ("bath", "2", "Salles de bain complètes"),
             ("land", "22 152 pi²", "Terrain"),
             ("barn", "Grange", "Allée pour 2 voitures")]
...
def slide3():
    c = base()
    photo(c, CUISINE, (0, 0, W, 700), r=0, shad=False, fy=0.5)
    d = ImageDraw.Draw(c)

    y = 756
    eyebrow(c, (M, y), "LES CUISINES")
    y += 46
    d.text((M, y), "Deux cuisines,", font=playfair(66, 700), fill=NAVY)
    d.text((M, y + 78), "une par étage", font=playfair(66, 700), fill=NAVY)
    y += 196

    for t in ["Cuisine de 14 x 13 pi avec coin repas",
              "Seconde cuisine et dînette à l'étage",
              "Pratique pour une famille élargie"]:
        icon(c, "check", (M, y + 2), 34, NAVY + (255,))
        d.text((M + 52, y), t, font=inter(29, 500), fill=INK)
        y += 62
...
# ================================================================ SLIDE 4
def slide4():
    """Les travaux deja faits, avec les annees. Cinq points, donc la photo
    est un peu plus courte et l interligne un peu plus serre."""
    c = base()
    photo(c, COTE, (0, 0, W, 640), r=0, shad=False, fy=0.55)
    d = ImageDraw.Draw(c)

    y = 700
    eyebrow(c, (M, y), "DÉJÀ FAIT")
    y += 46
    d.text((M, y), "Tranquillité d'esprit", font=playfair(68, 700), fill=NAVY)
    y += 120

    for t in ["Revêtement de vinyle refait en 2024",
        ...
        icon(c, "check", (M, y + 2), 32, NAVY + (255,))
        d.text((M + 50, y), t, font=inter(28, 500), fill=INK)
        y += 58
...
def slide5():
    ...
    photo(c, CHAMBRE, (M, 268, W - M, 700), fy=0.5)
    cw = (W - 2 * M - 24) // 2
    photo(c, CHAMBRE_PIN, (M, 724, M + cw, 1028), fy=0.5)
    photo(c, POELE, (M + cw + 24, 724, W - M, 1028), fy=0.5)

    d.text((M, 1058), "Du rez-de-chaussée à l'étage", font=inter(25, 700),
           fill=NAVY)
    para(d, (M, 1102),
         "Chambre principale au rez-de-chaussée, trois chambres à l'étage "
         "et un poêle à combustion lente.",
...
# ================================================================ SLIDE 6
def slide6():
    """La grange de bois, l image la plus parlante du terrain."""
    c = base()
    photo(c, GRANGE, (0, 0, W, 700), r=0, shad=False, fy=0.5)
    d = ImageDraw.Draw(c)

    y = 756
    eyebrow(c, (M, y), "SAINT-HERMAS, MIRABEL")
    y += 46
    d.text((M, y), "La campagne, pour vrai", font=playfair(66, 700), fill=NAVY)
    y += 122
    ...
    for ic, t in [("land", "Terrain de 22 152 pi², plat et gazonné"),
        ...
        d.text((M + 58, y), t, font=inter(28, 500), fill=INK)
        y += 62
...
def slide7():
    c = base()
    photo(c, GALERIE, (0, 0, W, H), r=0, shad=False, fy=0.5)
    ...
    d.text((W // 2, 972), "vivre ici.", font=playfair(80, 700), fill=WHITE,
           anchor="ma")
    tracked(d, (W // 2, 1094), ADRESSE_1L + "   ·   " + PRIX.upper(),
            inter(22, 700), (255, 255, 255, 215), 3, "c")
...
if __name__ == "__main__":
    ...
    for fn, name in [
            ...
            (slide3, "03-cuisines.jpg"), (slide4, "04-tranquillite.jpg"),
